...
const customerId = "c123"; // Later you can get from auth.
...
export const fetchCart = () => async (dispatch: AppDispatch) => {
  ...
  try {
    ...
    dispatch(fetchCartFailure(err?.response?.data?.message || "Failed to fetch cart"));
  }
};
...
export const addToCart = (item: any) => async (dispatch: AppDispatch) => {
  ...
  try {
    ...
    dispatch(addToCartFailure(err?.response?.data?.message || "Failed to add item"));
  }
};

export const updateCartItemQuantity = (itemId: string, quantity: number) => async (dispatch: AppDispatch) => {
  ...
  try {
    ...
    dispatch(updateQuantityFailure(err?.response?.data?.message || "Failed to update quantity"));
  }
};

export const removeCartItem = (itemId: string) => async (dispatch: AppDispatch) => {
  dispatch(removeItemStart());
  try {
    await orderApi.delete(`/cart/${customerId}/item/${itemId}`);
    dispatch(removeItemSuccess(itemId));
  } catch (err: any) {
    dispatch(removeItemFailure(err?.response?.data?.message || "Failed to remove item"));
  }
};
...
export const clearCart = () => async (dispatch: AppDispatch) => {
  ...
  try {
    ...
    dispatch(clearCartFailure(err?.response?.data?.message || "Failed to clear cart"));
  }
};
