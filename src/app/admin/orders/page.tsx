...
export default function AdminOrdersPage() {
  ...
  useEffect(() => {
    dispatch(fetchOrders("6803430128070e0251d7e0ea"));
  }, [dispatch]);
  ...
  const handleUpdateStatus = async (orderId: string) => {
    ...
    dispatch(fetchOrders("admin")); // Refresh orders after update
  };

  return (
    <div className="max-w-6xl mx-auto p-6 pt-24">
      <h1 className="text-3xl font-bold text-black mb-6 text-center">Manage Orders</h1>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {orders.map(order => (
            <div key={order._id} className="bg-white border border-yellow-300/50 rounded-xl p-4">
